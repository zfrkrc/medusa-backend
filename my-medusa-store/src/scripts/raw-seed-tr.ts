...
const MAJOR_DISTRICTS: Record<number, string[]> = {
    34: ["Kadıköy", "Beşiktaş", "Üsküdar", "Şişli", "Fatih", "Beyoğlu", "Bakırköy", "Ataşehir"],
    6: ["Çankaya", "Keçiören", "Yenimahalle", "Mamak", "Etimesgut", "Sincan", "Gölbaşı"],
    35: ["Konak", "Karşıyaka", "Bornova", "Buca", "Çiğli", "Gaziemir", "Balçova"]
}

export default async function rawSeed({ container }: ExecArgs) {
    const pgConnection = container.resolve(ContainerRegistrationKeys.PG_CONNECTION) as any
    const logger = container.resolve(ContainerRegistrationKeys.LOGGER)

    logger.info("📍 TURKISH ADDRESS SEEDING V2 - STARTING NOW...")

    for (const p of PROVINCES) {
        // Check if province exists
        const res = await pgConnection.raw(
            "SELECT id FROM tr_province WHERE plate_code = ?",
            [p.plate_code]
        )

        let provinceId = `prov_${p.plate_code}`
        if (res.rows.length === 0) {
            await pgConnection.raw(
                "INSERT INTO tr_province (id, name, plate_code, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())",
                [provinceId, p.name, p.plate_code]
            )
            logger.info(`  ✅ Created Province: ${p.name}`)
        } else {
            provinceId = res.rows[0].id
        }

        // Seed districts if it's a major city
        if (MAJOR_DISTRICTS[p.plate_code]) {
            for (const dName of MAJOR_DISTRICTS[p.plate_code]) {
                const dRes = await pgConnection.raw(
                    "SELECT id FROM tr_district WHERE name = ? AND province_id = ?",
                    [dName, provinceId]
                )

                if (dRes.rows.length === 0) {
                    const dId = `dist_${p.plate_code}_${dName.toLowerCase().replace(/ /g, "_")}`
                    await pgConnection.raw(
                        "INSERT INTO tr_district (id, name, province_id, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())",
                        [dId, dName, provinceId]
                    )
                    logger.info(`    🏙️ Created District: ${dName} in ${p.name}`)
                }
            }
        }
    }

    logger.info("🎉 Raw seeding completed!")
}
