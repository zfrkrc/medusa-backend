...
export default async function seedTrAddresses({ container }: ExecArgs) {
    ...
    logger.info("📍 Turkish Provinces seeding starts...")

    for (const p of PROVINCES) {
        const existing = await remoteQuery({
            tr_province: {
                fields: ["id"],
                where: { plate_code: p.plate_code }
            }
        })

        if (existing.length === 0) {
            await addressTrService.createTrProvinces(p)
            logger.info(`  ✅ Created: ${p.name} (${p.plate_code})`)
        }
    }

    const istanbulArr = await remoteQuery({
        tr_province: {
            fields: ["id"],
            where: { plate_code: 34 }
        }
    ...
    if (istanbulArr.length > 0) {
        const istanbul = istanbulArr[0]
        const demoDistricts = ["Kadıköy", "Beşiktaş", "Üsküdar", "Şişli"]
        for (const d of demoDistricts) {
            const existing = await remoteQuery({
                tr_district: {
                    fields: ["id"],
                    where: { name: d, province_id: istanbul.id }
                }
            })
            if (existing.length === 0) {
                await addressTrService.createTrDistricts({ name: d, province_id: istanbul.id })
                logger.info(`  🏙️ Added district ${d} to İstanbul`)
            }
        }
    }

    logger.info("🎉 Turkish Addresses seeding completed!")
}
